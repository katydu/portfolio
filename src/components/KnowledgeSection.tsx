      ...
      <div className="container mx-auto">
        ...
        <div className="flex justify-center mb-12 overflow-x-auto pb-4">
          <div className="flex space-x-3">
            {categories.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={`flex items-center px-4 py-2 rounded-full transition-colors ${activeCategory === category.id ? 'text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} style={activeCategory === category.id ? {
            backgroundColor: '#FAD'
          } : {}}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredItems.map(item => <div key={item.id} className="bg-gray-50 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4">
                <span className="inline-block bg-pink-100 px-3 py-1 rounded-full text-sm" style={{
              color: '#FAD'
            }}>
